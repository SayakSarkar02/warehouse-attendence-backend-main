...
attendancecvv = "warehouse1.csv"
studentsfolder = "warehouse1"
...
def update_class():
    global attendancecvv, studentsfolder, known_face_encodings, known_face_names
    data = request.get_json()
    class_name = data.get('class_name')
    if class_name:
        attendancecvv = f"{class_name}.csv"
        studentsfolder = class_name
        known_face_encodings = []  # Clear known face encodings
        known_face_names = []  # Clear known face names
        load_known_faces()  # Reload faces for the new class
...
def update_attendance_csv(student_name, present, checkin=None, checkout=None):
    ...
    if student_row is not None:
        ...
        if checkin:
            attendance_data[student_row][3] = checkin
        if checkout:
            attendance_data[student_row][4] = checkout
    else:
        # If the student is not found, create a new row
        new_row = [current_date, student_name, 'Present' if present else 'Absent', checkin if checkin else '', checkout if checkout else '']
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        if not attendance_data or attendance_data[0] != ['date', 'name', 'attendance', 'checkin', 'checkout']:
            writer.writerow(['date', 'name', 'attendance', 'checkin', 'checkout'])
...
def check_in():
    ...
    update_attendance_csv(student_name, present=True, checkin=datetime.now().strftime('%H:%M:%S'))
    return jsonify({"message": "Check-in time recorded"})

@app.route('/check-out', methods=['POST'])
def check_out():
    data = request.get_json()
    student_name = data.get('student_name')
    update_attendance_csv(student_name, present=True, checkout=datetime.now().strftime('%H:%M:%S'))
    return jsonify({"message": "Check-out time recorded"})
...
def upload_image():
    ...
    try:
        ...
        # Convert the numpy array to a BGR image (OpenCV format)
...
if __name__ == '__main__':
    app.run(debug=True, port=4005, host="0.0.0.0")
